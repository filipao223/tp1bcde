...
float searchDistance(float dMin, int num_pontos, CNode* start){
  ...
  i=1;
  for(current = start; current!=NULL && (i <= num_pontos); current = current->next){
    if( abs(current->x - midPoint->x) < dMin){
      if(new_head->next == NULL){
        new_head->next = malloc(sizeof(struct Coord));
        new_head->next->x = current->x;
        new_head->next->y = current->y;
        new_head->next->next = NULL;
        next = new_head->next->next;
      }
      else{
        next = malloc(sizeof(struct Coord));
        next->x = current->x;
        next->y = current->y;
        next->next = NULL;
        next = next->next;
